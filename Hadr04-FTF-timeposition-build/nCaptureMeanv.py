...
ss_res = np.sum((meanv - fit) ** 2)

# total sum of squares
ss_tot = np.sum((meanv - np.mean(meanv)) ** 2)
...
for i, txt in enumerate(meanv):
    if i>=9:
        plt.annotate(txt, (energies[i], meanv[i]), xytext=(3,-10), textcoords='offset pixels', fontsize=4)

plt.title('Mean v of nCapture', y=1.05)
plt.xlabel('Neutron Energy (MeV)')
plt.ylabel('v (m/microsec)')
plt.xlim(left=0)
plt.ylim(bottom=0)
plt.legend(loc=(0.7,0.5))
plt.savefig('images/nCapturemeanv.png', dpi=800, bbox_inches='tight')
plt.show()
...
for i in range(4):
    for j in range(5):
        if numplot<len(dflist):
            if numplot<10:
                ax[i, j].text(5, 1600, str((numplot+1)*10)+' MeV'+'\n'+str(len(dflist[numplot]['v']))+' entries', fontsize=4, ha='center')
            elif numplot>=10:
                ax[i, j].text(5, 1600, str((numplot-8)*100)+' MeV'+'\n'+str(len(dflist[numplot]['v']))+' entries', fontsize=4, ha='center')
            ax[i, j].scatter(dflist[numplot]['v'], dflist[numplot]['t'], s=0.1, edgecolors='none')
            ax[i, j].set_xlim(left=0, right=14)
            ax[i, j].set_ylim(bottom=0, top=2100)
            numplot+=1

fig.suptitle('v and Time of nCapture')
fig.text(0.5, 0.01, 'v (m/microsec)', ha='center', va='center')
fig.text(0.01, 0.5, 'Time (microsec)', ha='center', va='center', rotation='vertical')

plt.savefig('images/nCapturevvst.png', dpi=800, bbox_inches='tight')
plt.show()
...
for i in range(4):
    for j in range(5):
        if numplot<len(dflist):
            if numplot<10:
                ax[i, j].text(5, 10, str((numplot+1)*10)+' MeV'+'\n'+str(len(dflist[numplot]['v']))+' entries', fontsize=4, ha='center')
            elif numplot>=10:
                ax[i, j].text(5, 10, str((numplot-8)*100)+' MeV'+'\n'+str(len(dflist[numplot]['v']))+' entries', fontsize=4, ha='center')
            ax[i, j].scatter(dflist[numplot]['v'], dflist[numplot]['rdist'], s=0.1, edgecolors='none')
            ax[i, j].set_xlim(left=0, right=14)
...
fig.suptitle('v and Time of nCapture')
fig.text(0.5, 0.01, 'v (m/microsec)', ha='center', va='center')
fig.text(0.01, 0.5, 'Distance (m)', ha='center', va='center', rotation='vertical')

plt.savefig('images/nCapturevvsr.png', dpi=800, bbox_inches='tight')
plt.show()
...
for i in range(4):
    for j in range(5):
        if numplot<len(dflist):
            ...
            ax[i, j].hist(dflist[numplot]['v'], bins=100)
            ax[i, j].set_xlim(left=0, right=6)
            ax[i, j].set_ylim(bottom=0, top=1000)
            numplot+=1

fig.suptitle('v of nCapture Counts')
fig.text(0.5, 0.01, 'v (m/microsec)', ha='center', va='center')
fig.text(0.01, 0.5, 'Count (100 bins)', ha='center', va='center', rotation='vertical')

plt.savefig('images/nCapturevhisto.png', dpi=800, bbox_inches='tight')
plt.show()
